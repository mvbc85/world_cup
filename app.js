const LOCAL_STORAGE_KEY = "wc2026_sweepstake_state_v2";
const PARTICIPANTS_COLLECTION = "participants";
const ENTRY_PASSKEY = String(window.APP_ENTRY_PASSKEY || "").trim();
const PASSKEY_SESSION_STORAGE_KEY = "wc2026_passkey_access_v1";
const SAFE_NAME_PATTERN = /^[A-Za-z0-9][A-Za-z0-9 .,'()\-]{0,49}$/;
...
let useCloudPersistence = false;
let db = null;
let participantsUnsubscribe = null;
let hasPasskeyAccess = sessionStorage.getItem(PASSKEY_SESSION_STORAGE_KEY) === "true";
let selectedTeamName = null;
let filteredTeams = [];

const els = {
  form: document.getElementById("participant-form"),
  passkeyInput: document.getElementById("entry-passkey"),
...
function init() {
  setupRankedExplorer();
  setupCountdown();
  updatePasskeyUiStatus();

  els.form.addEventListener("submit", onAddParticipant);
  els.teamSearch.addEventListener("input", onSearchTeams);
  document.addEventListener("click", onRegisterActionClick);

  initPersistence();
}

function onRegisterActionClick(event) {
  const button = event.target.closest("button[data-action][data-id]");
  if (!button) {
    return;
  }

  if (!button.closest("#participant-register")) {
    return;
  }
...
function initPersistence() {
  ...
  useCloudPersistence = false;
  const localState = loadLocalState();
  state.participants = localState.participants;
  state.assignmentsDone = localState.assignmentsDone;
  setSyncStatus("Browser local storage");
  setPasskeyStatus(getPasskeyStatusMessage());
  renderParticipants();
  updatePot();
}
...
function initializeFirebase() {
  try {
    ...
    useCloudPersistence = true;

    setSyncStatus("Firebase (shared)");
    setPasskeyStatus(getPasskeyStatusMessage());
    ...
    state.assignmentsDone = localState.assignmentsDone;
    setSyncStatus("Browser local storage");
    setPasskeyStatus("Firebase failed to initialize. Using local browser storage with the frontend passkey gate.");
    renderParticipants();
    updatePot();
  }
}

function setSyncStatus(modeText) {
  if (els.syncMode) {
    els.syncMode.textContent = `Data source: ${modeText}`;
  }
}
...
function getPasskeyStatusMessage() {
  if (!ENTRY_PASSKEY || ENTRY_PASSKEY.includes("REPLACE_WITH_TEAM_PASSKEY")) {
    return "Host setup needed: add the shared passkey in firebase-config.js.";
  }

  return hasPasskeyAccess
    ? "Passkey accepted for this browser session."
    : "Enter the internal passkey once to unlock participant changes in this browser session.";
}

function updatePasskeyUiStatus() {
  if (!els.passkeyInput) {
    return;
  }
  ...
  if (hasPasskeyAccess) {
    ...
    els.passkeyInput.disabled = true;
    els.passkeyInput.placeholder = "Passkey accepted for this session";
    return;
  }

  els.passkeyInput.disabled = false;
  els.passkeyInput.placeholder = "Enter team passkey";
}

function clearPasskeyAccess() {
  hasPasskeyAccess = false;
  sessionStorage.removeItem(PASSKEY_SESSION_STORAGE_KEY);
  updatePasskeyUiStatus();
  setPasskeyStatus(getPasskeyStatusMessage());
}

function ensurePasskeyAccess(enteredPasskey) {
  if (hasPasskeyAccess) {
    return true;
  }

  if (!ENTRY_PASSKEY || ENTRY_PASSKEY.includes("REPLACE_WITH_TEAM_PASSKEY")) {
    window.alert("Passkey is not configured yet. Ask the host to set window.APP_ENTRY_PASSKEY in firebase-config.js.");
    return false;
  }

  const passkey = String(enteredPasskey || "").trim();
  if (!passkey) {
    window.alert("Enter the team passkey first.");
    return false;
  }

  if (passkey !== ENTRY_PASSKEY) {
    window.alert("Invalid passkey.");
    return false;
  }

  hasPasskeyAccess = true;
  sessionStorage.setItem(PASSKEY_SESSION_STORAGE_KEY, "true");
  updatePasskeyUiStatus();
  setPasskeyStatus(getPasskeyStatusMessage());
  return true;
}
...
function subscribeToParticipants() {
  ...
  participantsUnsubscribe = db.collection(PARTICIPANTS_COLLECTION).onSnapshot(
    (snapshot) => {
      state.participants = snapshot.docs
        .map((doc) => normalizeParticipant({ id: doc.id, ...doc.data() }))
    ...
    (error) => {
      ...
      window.alert("Could not load shared participants. Check Firebase rules and authorized domain settings.");
    }
  );
}
...
function sortParticipants(a, b) {
  if (a.createdAtMs && b.createdAtMs && a.createdAtMs !== b.createdAtMs) {
...
async function onAddParticipant(event) {
  ...
  const enteredPasskey = els.passkeyInput?.value.trim() || "";
  const name = normalizeParticipantName(els.nameInput.value);

  if (!ensurePasskeyAccess(enteredPasskey)) {
    return;
  }
  ...
  if (!isValidParticipantName(name)) {
    window.alert("Names can only use letters, numbers, spaces, and . , ' ( ) - characters, up to 50 characters.");
    return;
  }

  if (state.participants.some((participant) => participant.name.toLowerCase() === name.toLowerCase())) {
    window.alert("That name has already been added.");
    return;
  }
  ...
  try {
    if (useCloudPersistence) {
      await db.collection(PARTICIPANTS_COLLECTION).doc(participant.id).set({
    ...
    window.alert("Could not add participant. Please try again.");
    return;
  }
...
async function togglePaid(id) {
  ...
  if (!ensurePasskeyAccess(els.passkeyInput?.value.trim() || "")) {
    return;
  }

  try {
    if (useCloudPersistence) {
      await db.collection(PARTICIPANTS_COLLECTION).doc(id).update({
        paid: !participant.paid,
        updatedAt: window.firebase.firestore.FieldValue.serverTimestamp()
      });
    } else {
    ...
    window.alert("Could not update payment status. Please try again.");
  }
}
...
async function removeParticipant(id) {
  ...
  try {
    ...
    window.alert("Could not remove participant. Please try again.");
  }
}

async function assignTeamsToParticipant(id) {
  if (!ensurePasskeyAccess(els.passkeyInput?.value.trim() || "")) {
    return;
  }
  ...
  try {
    ...
    window.alert("Could not save team assignments. Please try again.");
  }
}
  ...
  return participants
    .map((participant) => {
      const teamsHtml = participant.teams.length
        ? participant.teams
          .map((teamName) => `<span class="team-chip">${escapeHtml(teamName)}</span>`)
          .join("")
      ...
      return `
        <tr>
          ...
          <td>${teamsHtml}</td>
          <td>
            <button class="ghost-btn" data-action="assign-manual" data-id="${participant.id}" type="button">Assign Teams</button>
